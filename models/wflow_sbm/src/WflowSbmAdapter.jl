...
const COLUMNS = ("time", "pr", "evspsbl", "mrro", "dis", "river_dis", "gwex", "mrso", "snw", "canopy", "channel")
...
function simulate(forcing::Forcing, static::AbstractDict, timestep::AbstractString,
    ...
    columns = zeros(n, 9)  # evspsbl mrro dis river_dis gwex mrso snw canopy channel
    ...
    for i in 1:n
        ...
        columns[i, 4] = river.q_av[1]
        columns[i, 5] = withdrawal ? -(leakage + removed) / dt_days : -leakage / dt_days
    ...
    notes = Dict{String, Any}(
        ...
        "reported" => Dict{String, Any}(
            ...
            "river_dis" => "native Wflow river q_av at the outlet gauge, m3/s; diagnostic only, excluding overland and lateral subsurface bypass flow",
